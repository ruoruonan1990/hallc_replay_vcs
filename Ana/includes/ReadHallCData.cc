...
const float CTtimeCut = 1.0; //This is the total size of the coincidence time window on on both positive and negative side. i.e. 1.5 would mean cut from -1.5 to 1.5;
const float CTbackLow = 7.0; //This is the lower limit of the background CT time window.  Note that this value applies to both positive and negative CTtime on either side of peak.
const float CTbackHi  = 13.0; //The upper limit of background CT time window.  Same as for above, this applies to both positive and negative.

const float CTSigBackRat = 2.0*CTtimeCut/(2.0*(CTbackHi - CTbackLow));  //The ratio to apply when subtracting coincidental background.
...
cout<< hms_beta_min<<" "<< hms_beta_max<<" "<<hms_beta_bins<<" "<<shms_beta_min<<" "<< shms_beta_max<<" "<<shms_beta_bins<<" "<<endl;
  ...
  HallCTree->Branch("misstimeref", &misstimeref, "misstimeref[22]/I");
	...
	cout<<"out tree init "<<HallCTree->GetEntries()<<endl;
		...
		if (process.compare("elastic")==0 || process.compare("vcs")==0 || process.compare("pi0")==0){
			if ((hms_cal_en_cut>0 && H_cal_etotnorm>hms_cal_en_cut) || (hms_cer_npe_cut>0 && H_cer_npeSum>hms_cer_npe_cut)
			   || (hms_beta_min>0 && H_gtr_beta<hms_beta_min) || (hms_beta_max>0 && H_gtr_beta>hms_beta_max) || H_tr_n != 1 
			...
			if ((shms_aero_npe_cut>0 && P_aero_npeSum<shms_aero_npe_cut) || (shms_hgcer_npe_cut>0 && P_hgcer_npeSum<shms_hgcer_npe_cut)
			    || (shms_cal_en_cut>0 && P_cal_etotnorm<shms_cal_en_cut) ||  P_tr_n != 1
   	...
   	if (process.compare("elasticLT")==0 || process.compare("vcsLT")==0 || process.compare("pi0LT")==0 
    ...
    } else if (process.compare("elastic")==0 || process.compare("vcs")==0 || process.compare("pi0")==0 
      ...
      W_kinmod = P_kin_primary_W;
		...
		} else if (fabs(CTime_epCoinTime_ROC2-time_shift)>CTbackLow && fabs(CTime_epCoinTime_ROC2-time_shift)<CTbackHi){
...
int ReadHallCData::DrawHist(string process, int run){
	...
	TLine *lCTlolo = new TLine(-CTbackHi,1.0,-CTbackHi,h_CTime_epCoinTime_ROC2[0]->GetMaximum()*1.05);
        lCTlolo->SetLineColor(kMagenta -1);
        lCTlolo->SetLineStyle(2);
        lCTlolo->Draw("same");

	TLine *lCTlohi = new TLine(-CTbackLow,1.0,-CTbackLow,h_CTime_epCoinTime_ROC2[0]->GetMaximum()*1.05);
       	lCTlohi->SetLineColor(kMagenta -1);
       	lCTlohi->SetLineStyle(2);
       	lCTlohi->Draw("same");

	TLine *lCThilo = new TLine(CTbackLow,1.0,CTbackLow,h_CTime_epCoinTime_ROC2[0]->GetMaximum()*1.05);
       	lCThilo->SetLineColor(kMagenta -1);
       	lCThilo->SetLineStyle(2);
       	lCThilo->Draw("same");

	TLine *lCThihi = new TLine(CTbackHi,1.0,CTbackHi,h_CTime_epCoinTime_ROC2[0]->GetMaximum()*1.05);
       	lCThihi->SetLineColor(kMagenta -1);
       	lCThihi->SetLineStyle(2);
       	lCThihi->Draw("same");

	TLine *lCTGoodLo = new TLine(-CTtimeCut,1.0,-CTtimeCut,h_CTime_epCoinTime_ROC2[0]->GetMaximum()*1.05);
       	lCTGoodLo->SetLineColor(kMagenta +1);
       	lCTGoodLo->SetLineStyle(2);
       	lCTGoodLo->Draw("same");

	TLine *lCTGoodHi = new TLine(CTtimeCut,1.0,CTtimeCut,h_CTime_epCoinTime_ROC2[0]->GetMaximum()*1.05);
  ...
  c1->SaveAs(Form(VCS_REPLAY_PATH "/Ana/Results/ana_monitor_%d.pdf",run)); 
	...
	c1->Clear();c1->Divide(2,2);
	c1->cd(1); 
	h_M2miss[2]->SetLineColor(9); h_M2miss[2]->Draw("HIST"); gPad->SetLogy(0); h_M2miss[2]->SetMinimum(0);
	h_M2miss[4]->Draw("HISTsame"); h_M2miss[3]->SetLineColor(8); h_M2miss[3]->Draw("HISTsame");
	c1->cd(2); 
	h_Mmiss[2]->SetLineColor(9); h_Mmiss[2]->Draw("HIST"); gPad->SetLogy(0); h_Mmiss[2]->SetMinimum(0);
	h_Mmiss[4]->Draw("HISTsame"); h_Mmiss[3]->SetLineColor(8); h_Mmiss[3]->Draw("HISTsame"); 
	c1->cd(3); 
	h_Emiss[2]->SetLineColor(9); h_Emiss[2]->Draw("HIST"); gPad->SetLogy(0); h_Emiss[2]->SetMinimum(0);
	h_Emiss[4]->Draw("HISTsame"); h_Emiss[3]->SetLineColor(8); h_Emiss[3]->Draw("HISTsame"); 
	c1->cd(4); 
	h_PTmiss[2]->SetLineColor(9); h_PTmiss[2]->Draw("HIST"); gPad->SetLogy(0); h_PTmiss[2]->SetMinimum(0);
	...
	h_M2miss[4]->Draw("HISTsame"); h_M2miss[3]->SetLineColor(8); h_M2miss[3]->Draw("HISTsame");
	c1->cd(2); 
	h_gmom[2]->SetLineColor(9); h_gmom[2]->Draw("HIST"); gPad->SetLogy(0); h_gmom[2]->SetMinimum(0);
	h_gmom[4]->SetLineColor(2); h_gmom[4]->Draw("HISTsame"); h_gmom[3]->SetLineColor(8); h_gmom[3]->Draw("HISTsame"); 
	c1->cd(3); 
	h_Emiss[2]->SetLineColor(9); h_Emiss[2]->Draw("HIST"); gPad->SetLogy(0); h_Emiss[2]->SetMinimum(0);
	h_Emiss[4]->Draw("HISTsame"); h_Emiss[3]->SetLineColor(8); h_Emiss[3]->Draw("HISTsame"); 
  c1->SaveAs(Form(VCS_REPLAY_PATH "/Ana/Results/ana_monitor_%d.pdf",run)); 
  ...
  c1->Clear();
	c1->Divide(2,1); 
	c1->cd(1); h2_Q2W[4]->Draw("HISTcolz"); 
	c1->cd(2); h2_ThCMPhi[4]->Draw("HISTcolz");
	c1->SaveAs(Form(VCS_REPLAY_PATH "/Ana/Results/ana_monitor_%d.pdf)",run)); 
